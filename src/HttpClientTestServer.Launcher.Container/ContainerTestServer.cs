...
namespace HttpClientTestServer.Launcher
{
    public class ContainerTestServer : ITestServer
    {
        ...
        private ContainerTestServer(TestServerOptions options, ILoggerProvider loggerProvider, CancellationToken cancellationToken)
        {
            ...
            var sslProtocols = options.SslProtocols ?? (SslProtocols)12288 /*SslProtocols.Tls13*/;
        ...
        public static async Task<ITestServer> LaunchAsync(TestServerOptions options, ILoggerProvider loggerProvider = null, CancellationToken shutdownToken = default)
        {
            ...
            var shutdownCancelTcs = new TaskCompletionSource<bool>();
            using var registration = shutdownToken.Register(() => shutdownCancelTcs.TrySetCanceled());
            
            await Task.WhenAny(server._waitForServerStartedTask, shutdownCancelTcs.Task);
        ...
        public async ValueTask DisposeAsync()
        {
            ...
            {
                var shutdownCancelTcs = new TaskCompletionSource<bool>();
                using var registration = new CancellationTokenSource(TimeSpan.FromSeconds(1)).Token.Register(() => shutdownCancelTcs.TrySetCanceled());

                await Task.WhenAny(_appTask, shutdownCancelTcs.Task);
            }
            catch
            {
                // ignore
